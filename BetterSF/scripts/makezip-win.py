...
def main():
  ...
  vst3_bundle = build_dir + "\\BetterSF.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\BetterSF Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\BetterSF Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\build-win\\manual\\BetterSF manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\BetterSF.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "BetterSF.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\BetterSF_x64.exe",
      projectpath + "\\build-win\\BetterSF_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\BetterSF_x64.clap", "BetterSF_x64.clap"),
      (projectpath + "\\build-win\\BetterSF_ARM64EC.clap", "BetterSF_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\BetterSF.clap", "BetterSF_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\BetterSF.clap", "BetterSF_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\BetterSF-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\BetterSF-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\BetterSF-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\BetterSF-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\BetterSF-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\BetterSF-clap_ARM64EC.pdb"
  ]
